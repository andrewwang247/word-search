...
/**
 * The CharGrid class holds the word search space containing the letter to be
 * searched. It allows for quick character retrieval and toggling of upper/lower
 * cases. Furthermore, the API supports adjacent searches that include bound
 * checking. The first vector dimension is the row number (up/down). The second
 * is the column.
 */
class CharGrid {
 public:
 ...
 private:
  ...
  struct Record {
    Record(size_t row_in, size_t col_in, Direction dir_in);
    size_t row, col;
    Direction dir;
  };
  ...
  // The target string + the original input string for printing.
  std::string target, original;

  // Processes str such that it is in the correct format for insertion.
  std::string process_string(std::string str) const;
